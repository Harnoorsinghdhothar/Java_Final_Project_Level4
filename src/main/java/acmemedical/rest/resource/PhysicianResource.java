...
public class PhysicianResource {
	...
	@RolesAllowed({ADMIN_ROLE})
	public Response updatePhysician(@PathParam(MyConstants.RESOURCE_PATH_ID_ELEMENT) int id, Physician updatedPhysician) {
		Response result = null;
		Physician updatedPhysicianWithIdTimestamps = null;
		try {
			updatedPhysicianWithIdTimestamps = service.updatePhysicianById(id, updatedPhysician);
		} catch (Exception e) {
			LOG.error("caught exception attempting to update physician", e);
			e.printStackTrace();
		}

		if (updatedPhysicianWithIdTimestamps == null) {
			LOG.debug("updatedPhysicianWithIdTimestamps is null");
		}
		else {
			LOG.debug("updated Physician : " + updatedPhysicianWithIdTimestamps);
		}

		//    	return Response.status(result == null ? Status.NOT_MODIFIED : Status.OK).entity(updatedCourseWithTimestamps).build();
		return Response.ok(updatedPhysicianWithIdTimestamps).build();
	}
