...
public class ACMEMedicalService implements Serializable {
    ...
    public void deletePhysicianById(int id) {
        LOG.debug("Starting deletion process for physician with ID: {}", id);

        // Fetch the physician by ID
        Physician physician = getPhysicianById(id);
        if (physician == null) {
            LOG.debug("No physician found with ID: {}", id);
            return;
        }

        LOG.debug("Physician found: {}", physician);

        try {
            // Fetch the associated SecurityUser entity
            ...
            if (sUser != null) {
                em.remove(sUser);
                LOG.debug("Successfully removed associated SecurityUser: {}", sUser);
            }

            // Remove the physician
            em.remove(physician);
            LOG.debug("Successfully removed physician: {}", physician);

        } catch (jakarta.persistence.NoResultException e) {
            LOG.debug("No associated SecurityUser found for physician with ID: {}", id);
        } catch (Exception e) {
            LOG.error("Exception occurred while deleting physician with ID: {}", id, e);
            throw new RuntimeException("Failed to delete physician due to unexpected error.", e);
        }

        LOG.debug("Deletion process completed for physician with ID: {}", id);
    }
 ...
 // Method to get all prescriptions
    ...
    public void deletePatientById(int id) {
        ...
        // Log and delete the patient
        LOG.debug("Patient to delete found: " + patientToBeDeleted);
